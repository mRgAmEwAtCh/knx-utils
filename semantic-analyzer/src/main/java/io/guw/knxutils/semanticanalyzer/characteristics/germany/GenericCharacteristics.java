...
public class GenericCharacteristics extends KnxProjectCharacteristicsAdapter {
	...
	GroupAddress searchForStatusInPattern2(GroupAddress primarySwitchGroupAddress, DatapointType datapointType) {
		GroupAddress candidate;
		...
		for (GroupAddressRange statusRange : statusRanges) {
			...
			if (candidate != null) {
				log.debug("Evaluating potential candidate for GA {}: {}", primarySwitchGroupAddress, candidate);
				if (isMatchOnNameAndDpt(candidate, primarySwitchGroupAddress, datapointType)) {
